...
// Execute tests.
simpletest_script_execute_batch($test_id, simpletest_script_get_test_list(), $args['recreate-db']);
 ...
 */
function simpletest_script_execute_batch($test_id, $test_classes, $newdb = false) {
  global $args, $db_test_map, $testing_dbs;
  ...
  //Create dump
  if ($newdb) {
    $command = 'drush sql-dump --result-file=/tmp/digipolis.sql';
    proc_open($command, array(), $pipes, NULL, NULL, array('bypass_shell' => TRUE));

    foreach ($testing_dbs as $key => $db) {
      $sql = "DROP DATABASE " . $key;
      $command = "echo \"$sql\" | mysql -u$dbuser -p$dbpassword";
      proc_open($command, array(), $pipes, NULL, NULL, array('bypass_shell' => TRUE));

      $sql = str_replace("#DOMAIN#", $key, "
      ...
      $command = "mysql -u$dbuser -p$dbpassword $key < /tmp/digipolis.sql";
      proc_open($command, array(), $pipes, NULL, NULL, array('bypass_shell' => TRUE));
      print 'Database created ' . $key . "\n";
    }
  }
  ...
  while (!empty($test_classes) || !empty($children)) {
    while (count($children) < $args['concurrency']) {
      ...
      if (!empty($db_out)) {
        setconnection($db_out, $dbuser, $dbpassword);
        $testing_dbs[$db_out] = array('status' => 1);
        print('Db connection created ' . $db_out . "\n");
      }
      else {
        break;
      }
      ...
      print_r('Started ' . $test_class . ' with db ' . $db_out . "\n");
      $db_test_map[$db_out][] = $test_class;
    ...
    foreach ($children as $cid => $child) {
      ...
      if (empty($status['running'])) {
        ...
        //Release database
        unset($testing_dbs[$child['db']]['status']);
 ...
 */
function setconnection($db, $dbuser, $dbpassword) {
  $other_database = array(
    'database' => $db,
    'username' => $dbuser, // assuming this is necessary
    'password' => $dbpassword, // assuming this is necessary
    'host' => 'localhost', // assumes localhost
    'driver' => 'mysql', // replace with your database driver
  );
  // replace 'YourDatabaseKey' with something that's unique to your module
  Database::addConnectionInfo($db, 'default', $other_database);
  db_set_active($db);
}
...
function simpletest_script_reporter_display_results() {
  global $args, $test_id, $results_map, $db_test_map;
